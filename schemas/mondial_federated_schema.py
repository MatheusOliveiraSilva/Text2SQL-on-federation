...
# Abaixo está um teste para ver se o LLM gerou todas as tabelas corretamente.
def tables_checks(database_schema):
    for tables in database_schema.split("CREATE TABLE"):
        table_name = tables.split("\n")[0]

        for table in all_tables_names:
            if table == table_name:
                all_tables_names.pop(all_tables_names.index(table_name))

if __name__ == "__main__":
    all_tables_names = []

    for table in MONDIAL_FULL_SCHEMA.split("CREATE TABLE"):
        table_name = table.split("\n")[0]
        if table:
            all_tables_names.append(table_name)

    tables_checks(MONDIAL_ECONOMY)
    tables_checks(MONDIAL_SOCIAL)
    tables_checks(MONDIAL_GEO)
    tables_checks(MONDIAL_POlITICS)
